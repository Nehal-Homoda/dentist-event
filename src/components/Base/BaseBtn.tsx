...
export default function BaseBtn({ children ,isWithArrow=true, minWidth = '',inverted = false}: Props) {
    return (
            <button className={`group flex-grow-0 flex-shrink-0  rounded-full   font-semibold px-6 text-sm h-11 ${minWidth} ${inverted ? 'bg-white text-primary' : 'bg-gradient-to-r from-primary to-primary-light text-white'}`}>
                {children}
                {isWithArrow && (
